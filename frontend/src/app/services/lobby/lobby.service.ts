...
export class LobbyService {
  ...
  createLobby(leaderId: number, gameId: number, playWithPoints: boolean): Observable<Lobby> {
    const params = new HttpParams()
      .set('leaderId', leaderId.toString())
      .set('gameId', gameId.toString())
      .set('playWithPoints', playWithPoints.toString());

    return this.http.post<Lobby>(`${this.apiUrl}/create`, null, { params })
      .pipe(
        tap(lobby => this.currentLobbySubject.next(lobby))
      );
  }
  ...
  joinLobby(code: string, userId: number): Observable<Lobby> {
    ...
    return this.http.post<Lobby>(`${this.apiUrl}/join`, null, { params })
      .pipe(
        tap(lobby => this.currentLobbySubject.next(lobby))
      );
  }
  ...
  kickPlayer(lobbyId: number, leaderId: number, playerId: number): Observable<Lobby> {
    ...
    return this.http.post<Lobby>(`${this.apiUrl}/kick`, null, { params })
      .pipe(
        tap(lobby => {
  ...
  leaveLobby(lobbyId: number, userId: number): Observable<any> {
    ...
    return this.http.post<any>(`${this.apiUrl}/leave`, null, { params })
      .pipe(
        tap(response => {
  ...
  updateLobbySettings(lobbyId: number, leaderId: number, gameId: number, playWithPoints: boolean): Observable<Lobby> {
    ...
    return this.http.put<Lobby>(`${this.apiUrl}/update-settings`, null, { params })
      .pipe(
        tap(lobby => {
          if (this.currentLobbySubject.value?.id === lobbyId) {
            this.currentLobbySubject.next(lobby);
  ...
  startGame(lobbyId: number, leaderId: number): Observable<Lobby> {
    ...
    return this.http.post<Lobby>(`${this.apiUrl}/start-game`, null, { params })
      .pipe(
        tap(lobby => {
          if (this.currentLobbySubject.value?.id === lobbyId) {
            this.currentLobbySubject.next(lobby);
  ...
  getLobby(lobbyId: number): Observable<Lobby> {
    ...
    return this.http.get<Lobby>(`${this.apiUrl}/get`, { params })
      .pipe(
        tap(lobby => {
